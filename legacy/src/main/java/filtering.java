public class GlobalMembersFiltering
{
...
// ***************************************************************
	...
	public static void rangefilter(input_gen generalinput, slcimage master, slcimage slave, productinfo interferogram, input_filtrange filtrangeinput)
	{
	  ...
	  // ====== Handle input ======
	  final real8 hamming = filtrangeinput.hammingalpha; // alpha=1: const
	  final int32 nlmean = filtrangeinput.nlmean; // odd number range lines to average
	  final int32 fftlength = filtrangeinput.fftlength; // length of adaptive
	  final int32 OVERLAP = filtrangeinput.overlap; // overlap input blocks
	  final real8 SNRthreshold = filtrangeinput.SNRthreshold; // criterium to filter
	  final boolean doweightcorrel = filtrangeinput.doweightcorrel;
	  final int32 oversamplefactor = filtrangeinput.oversample;

	  // RSR,RBW in Hz
	  final real8 RSR = 0.5 *master.rsr2x; // range sampling rate fr 18.96MHz
	  final real8 RBW = master.rbw; // range band width fs 15.55MHz
	  ...
	  final int32 numpixels = interferogram.win.pixels();

	  // ______ Memory buffers ______
	  final uint BUFFERMEMSIZE = generalinput.memory;
	  final int32 bytesperline = numpixels * sizeof(complr4);
	  final real4 numbigmatrices = 4.2; // in/out Master/Slave +rest
	  int32 bufferlines = (BUFFERMEMSIZE/numbigmatrices)/bytesperline;
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  final int32 numrangeblocks = numpixels/(fftlength-2 *OVERLAP); // approx
	  ...
	  window filteredpart = new window(0, bufferlines-uint((nlmean+1)/2), 0, numpixels-1); // output to disk

	  // ______ Give info ______
	  INFO << "overlap master slave: [" << interferogram.win.linelo << ":" << interferogram.win.linehi << "; " << interferogram.win.pixlo << ":" << interferogram.win.pixhi << "] (master system)";
	  INFO.print();
	  ...
	  for (azimuthbuffer =1; azimuthbuffer<1e5; ++azimuthbuffer)
		{
		  PROGRESS << "azimuthbuffer (" << bufferlines << " lines): " << azimuthbuffer << ".";
		  PROGRESS.print();
		  ...
		  if (endlinethisblock>=int32(interferogram.win.linehi)-nlmean-int32(bufferlines/10))
		{
		  ...
		  final window winfile = new window(startlinethisblock, endlinethisblock, interferogram.win.pixlo, interferogram.win.pixhi);
		  // ______ Input buffer ______
		  final matrix<complr4> MASTER_ORIG = master.readdata(winfile);
		  final matrix<complr4> SLAVE_ORIG = slave.readdata(winfile);
		  // ______ Output buffer ______
		  matrix<complr4> MASTER_FILT = new matrix(MASTER_ORIG.lines(),MASTER_ORIG.pixels());
		  matrix<complr4> SLAVE_FILT = new matrix(SLAVE_ORIG.lines(), SLAVE_ORIG.pixels());
		  ...
		  boolean rangedone = false; // not finished yet
		  for (;;) // forever
		{
		  ...
		  TRACE << "range block [" << startlinethisblock << ":" << endlinethisblock << "; " << startpixel+interferogram.win.pixlo << ":" << endpixel+interferogram.win.pixlo << "]";
		  TRACE.print();

		  // ______ Cut out master/slave for adaptive range filtering ______
		  final window win = new window(0,MASTER_ORIG.lines()-1,startpixel,endpixel);
		  matrix<complr4> PARTMASTER = new matrix(win,MASTER_ORIG); // construct as part
		  matrix<complr4> PARTSLAVE = new matrix(win,SLAVE_ORIG); // construct as part

		  // ______ Do the actual range filtering ______
		  // ______ filtered lines: index[0:L-1]: [(nlmean-1)/2:L-1-(nlmean-1)/2] ______
		  real8 SNRmean;
		  real8 percentnotfiltered;
		  RefObject<matrix<complr4>> TempRefObject = new RefObject<matrix<complr4>>(PARTMASTER);
		  RefObject<matrix<complr4>> TempRefObject2 = new RefObject<matrix<complr4>>(PARTSLAVE);
		  RefObject<real8> TempRefObject3 = new RefObject<real8>(SNRmean);
		  RefObject<real8> TempRefObject4 = new RefObject<real8>(percentnotfiltered);
		  rfilterblock(TempRefObject, TempRefObject2, nlmean, SNRthreshold, RSR, RBW, hamming, oversamplefactor, doweightcorrel, TempRefObject3, TempRefObject4); // statistics returned
		  PARTMASTER = TempRefObject.argvalue;
		  PARTSLAVE = TempRefObject2.argvalue;
		  SNRmean = TempRefObject3.argvalue;
		  percentnotfiltered = TempRefObject4.argvalue;
		  overallSNR += SNRmean;
		  overallnotfiltered += percentnotfiltered;

		  // ______ Put filtered part in outputbuffer ______
		  final window winpart = new window(win.linelo,win.linehi, partpixlo,partpixhi);
		  final window wintoset = new window(win.linelo,win.linehi, filteredpixlo,filteredpixhi);
		...
		case FORMATCI2:
		  {
			matrix TMP = new matrix(MASTER_FILT.lines(),MASTER_FILT.pixels());
	  ...
	  INFO.print();
	  INFO << "Filtered (approx): " << setprecision(3) << 100.00-overallnotfiltered << "%";
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  for (int32 fileID =1; fileID<=2; ++fileID)
		{
		  String oresfile = new String(new char[EIGHTY]);
		  String odatafile = new String(new char[EIGHTY]);
		  String odataformat = new String(new char[EIGHTY]);
		  String processcf = new String(new char[EIGHTY]);
		  if (filtrangeinput.oformatflag==FORMATCR4)
		odataformat = "complex_real4";
		  else if (filtrangeinput.oformatflag==FORMATCI2)
		odataformat = "complex_short";
		  else
		{
		...
		{
		case 1: // master
		  oresfile = "scratchresMfiltrange";
		  odatafile = filtrangeinput.fomaster;
		  processcf = processcontrol[pr_m_filtrange]; // control flag
		  break;
		case 2: // slave
		  oresfile = "scratchresSfiltrange";
		  odatafile = filtrangeinput.foslave;
		  processcf = processcontrol[pr_s_filtrange]; // control flag
		  break;
		default:
		  {
		  ...
		  ofstream scratchresfile = new ofstream(oresfile, ios.out | ios.trunc);
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
...
// ***************************************************************
	...
	public static void rfilterblock(RefObject<matrix<complr4>> master, RefObject<matrix<complr4>> slave, int32 nlmean, real8 SNRthreshold, real8 RSR, real8 RBW, real8 alphahamming, int32 osfactor, boolean doweightcorrel, RefObject<real8> meanSNR, RefObject<real8> percentnotfiltered)
	{
	  final int32 numlines = master.argvalue.lines();
	  final int32 numpixs = master.argvalue.pixels();
	  final int32 outputlines = numlines - nlmean + 1;
	  final int32 firstline = (nlmean-1)/2; // indices in matrix system
	  final int32 lastline = firstline + outputlines - 1;
	  final boolean dohamming = (alphahamming<0.9999) ? true : false;
	  // use oversampling before int. gen.
	  final boolean dooversample = (osfactor!=1) ? true : false;
	...
	//#if __DEBUG
	  ...
	  if (slave.argvalue.lines()!=numlines)
		{
		  PRINT_ERROR("slave not same size as master.")
		throw(argument_error);
		}
	  if (slave.argvalue.pixels()!=numpixs)
		{
		  PRINT_ERROR("slave not same size as master.")
		throw(argument_error);
		}
...
//ORIGINAL LINE: register int32 i,j;
	  int32 i;
	  int32 j;
	  final real4 deltaf = RSR/real4(numpixs);
	  final real4 fr = -RSR/2.;
	  matrix<real4> freqaxis = new matrix(1,numpixs);
	  for (i =0; i<numpixs; ++i)
	  ...
	  matrix<complr4> cint;
	  if (dooversample)
		cint = dotmult(oversample(master.argvalue,1,osfactor),oversample(conj(slave.argvalue),1,osfactor));
	  else
		cint = dotmult(master.argvalue,conj(slave.argvalue));
	  final int32 fftlength = cint.pixels();
	  ...
	  if (doweightcorrel)
		{
		  ...
		  final int32 indexnopeak = (1.-(RBW/RSR))*real4(numpixs);
		  for (j =0; j<fftlength; ++j)
		{
		  final int32 npnts = Math.abs(numpixs-j);
		  final real4 weight = (npnts<indexnopeak) ? sqr(numpixs) : sqr(npnts); // ==zero
	  ...
	  fft(master.argvalue,2); // master=fft over rows
	  fft(slave.argvalue,2); // slave=fft over rows
	  ...
	  for (register int32 oline =firstline; oline<=lastline; ++oline)
		{
		  ...
		  final real4 totalpower = totalp(0,0);
		  final real4 maxvalue = max(nlmeanpower,dummy,shift); // shift returned
		  uint lastshift = shift; // use this if current shift not ok.
		  final real8 SNR = fftlength*(maxvalue/(totalpower-maxvalue));
		  meanSNR.argvalue += SNR;
		  ...
		  if (SNR<SNRthreshold)
		{
		  ...
		  filter = myhamming(freqaxis-real4(.5 *shift *deltaf), RBW-real8(shift *deltaf), RSR,alphahamming); // fftshifted
		  filter *= inversehamming;
			}
		  else // no weighting of spectra
			{
		  filter = myrect((freqaxis-real4(.5 *shift *deltaf)) / (real4(RBW-shift *deltaf))); // fftshifted
			}
		  ...
		  if (negshift ==false)
			{
		  dotmult(master.argvalue[oline],filter,1);
		  filter.fliplr(); // works on data!
		  dotmult(slave.argvalue[oline],filter,1);
			}
		  else
			{
		  dotmult(slave.argvalue[oline],filter,1);
	  ...
	  ifft(master.argvalue,2); // master=ifft over rows
	  ifft(slave.argvalue,2); // slave=ifft over rows

	  // ______ Return these to main ______
	  meanSHIFT /= (outputlines-notfiltered);
	  meanSNR.argvalue /= outputlines;
	  percentnotfiltered.argvalue = 100.* (real4(notfiltered)/real4(outputlines));


	  // ______ Some info for this block ______
	  final real8 meanfrfreq = meanSHIFT *deltaf; // Hz?
	  DEBUG << "mean SHIFT for block" << ": " << meanSHIFT << " = " << meanfrfreq/1e6 << " MHz (fringe freq.).";
	  DEBUG.print();
	  DEBUG << "mean SNR for block" << ": " << meanSNR.argvalue;
	  DEBUG.print();
	  DEBUG << "filtered for block" << ": " << setprecision(3) << 100.00-percentnotfiltered.argvalue << "%" << ends;
	  DEBUG.print();
	  if (percentnotfiltered.argvalue>60.0)
		{
...
// ***************************************************************
	...
	public static void rangefiltporbits(input_gen generalinput, input_filtrange filtrangeinput, input_ell ellips, slcimage master, slcimage slave, RefObject<orbit> masterorbit, RefObject<orbit> slaveorbit)
	{
	  TRACE_FUNCTION("rangefiltporbits (BK 13-Nov-2000)")

		// ______ RSR,RBW in Hz, SI ______
		final real8 HAMMINGA = filtrangeinput.hammingalpha;
	  final boolean dohamming = (HAMMINGA<0.9999) ? true : false;
	  final real8 RSR = 0.5 *master.rsr2x; // range sampling rate fr 18.96MHz
	  final real8 RBW = master.rbw; // range band width fs 15.55MHz
	  ...
	  TRACE << "overlap window: " << overlap.linelo << " " << overlap.linehi << " " << overlap.pixlo << " " << overlap.pixhi;
	  TRACE.print();

	  // ______ Initialize loop parameters etc. ______
	  int32 FFTLENGTH = filtrangeinput.fftlength;
	  final int32 NCOLS = overlap.pixels(); // width 2b processed
	  while (FFTLENGTH>NCOLS)
		{
		  WARNING << "FFTLENGTH>overlap: New length: " << FFTLENGTH << " --> " << FFTLENGTH/2;
		  WARNING.print();
		  FFTLENGTH /= 2;
		}
	  final int32 NBLOCKS = (NCOLS/FFTLENGTH);
	  final int32 EXTRA = ((NCOLS%FFTLENGTH)==0) ? 0:1; // last overlaps previous
	  ...
	  INFO << "rangefiltporbits: MAXITER: " << MAXITER << "; " << "CRITERPOS: " << CRITERPOS << " m; " << "CRITERTIM: " << CRITERTIM << " s";
	  INFO.print();
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  // ______ Shift parameters ______
	  //register int32 i;
	  final real4 df = RSR/real4(FFTLENGTH);
	  final real4 fr = -RSR/2.;
	  matrix<real4> freqaxis = new matrix(1,FFTLENGTH);
	  ...
	  DEBUG << "parameters used: NCOLS " << NCOLS << " " << "FFTLENGTH: " << FFTLENGTH << " " << "NBLOCKS: " << NBLOCKS << " " << "EXTRA: " << EXTRA;
	  DEBUG.print();
	  DEBUG << "overlap window: " << overlap.linelo << " " << overlap.linehi << " " << overlap.pixlo << " " << overlap.pixhi;
	  DEBUG.print();

	  // ______ Progress messages ______
	  int32 percent = 10;
	  int32 tenpercent = (overlap.lines()/10)+.5; // round
	  ...
	  for (int32 line_m =overlap.linelo; line_m<=int32(overlap.linehi); ++line_m) // in master coord.
		{
		  ...
		  final real8 m_tazi = master.line2ta(line_m);
		  M = masterorbit.argvalue.getxyz(m_tazi);
		  ...
		  for (int32 rangeblock =1; rangeblock<=NBLOCKS+EXTRA; ++rangeblock)
		{
		  ...
		  final real8 middlepix = real8(pixlo_m+pixhi_m)/2.; // theta,Bperp here
		  // better compute range with m_trange...
		  lp2xyz(line_m,middlepix,ellips,master,masterorbit.argvalue,P,MAXITER,CRITERPOS);
		  // ______ Compute xyz slave satellite from P ______
		  real8 s_trange;
		  real8 s_tazi;
		  xyz2t(s_tazi,s_trange,slave,slaveorbit.argvalue,P,MAXITER,CRITERTIM);
		  // ______ Slave position ______
		  S = slaveorbit.argvalue.getxyz(s_tazi);
		  cn MP = M.min(P);
		  cn SP = S.min(P);
		  final real8 incidence1 = MP.angle(P); // theta1
		  final real8 incidence2 = SP.angle(P); // theta2
		  final real8 deltatheta = incidence1-incidence2; // counterclockwise
		  // ______ (approx. in Hz, see also manual) ______
		  final real4 deltaf = new real4((-SOL *deltatheta) / (master.wavelength *Math.tan(incidence1-filtrangeinput.terrainslope)));
		  totaldeltaf += deltaf;

		  //cerr << "deltaf: " << deltaf << " Hz\n";
		  // ______ Read block from disk ______
		  final int32 pixlo_s = pixlo_m + slave.coarseoffsetP;
		  final int32 pixhi_s = pixhi_m + slave.coarseoffsetP;
		  final window win_m = new window(line_m,line_m,pixlo_m,pixhi_m);
		  final window win_s = new window(line_s,line_s,pixlo_s,pixhi_s);
		  ...
		  if (dohamming)
			{
			  ...
			  FILTER = myhamming(freqaxis-real4(.5 *deltaf), RBW-deltaf, RSR,HAMMINGA); // fftshifted
			  FILTER *= inversehamming; // rect f. included
			}
		  else // no weighting of spectra
			{
			  FILTER = myrect((freqaxis-real4(.5 *deltaf)) / real4(RBW-deltaf)); // fftshifted
			}
		  ifftshift(FILTER); // fftsh works on data!
	...
	//#if WIN32
		  ...
		  FILTER.fliplr();
		  SLAVE = timesCxR(SLAVE,FILTER); // Jia ifft(MASTER,2); // back to space domain
	//#else
		  MASTER *= FILTER; // filtered master range spectral domain
		  FILTER.fliplr();
		  SLAVE *= FILTER; // filtered slave range spectral domain ifft(MASTER,2); // back to space domain
	//#endif
	//#else
		  WARNING.print("This is not as theory, sign Bperp defined otherwise?");
		  SLAVE *= FILTER; // filtered slave range spectral domain
		  FILTER.fliplr();
		  MASTER *= FILTER; // filtered master range spectral domain ifft(MASTER,2); // back to space domain
	//#endif
		  ...
		  if (lastblock ==false)
			...
			case FORMATCI2:
			  {
				matrix TMP = new matrix(1,MASTER.pixels());
		  ...
		  // ______ Filtered part F(NUMBLOCKS*FFTLENGTH:pixels()-1) ______
			...
			{
			  final int32 lastwritten = NBLOCKS *FFTLENGTH;
			  final int32 numtowrite = overlap.pixels() - lastwritten;
			  final window wintmp = new window(0,0,MASTER.pixels()-numtowrite,MASTER.pixels()-1);
			  matrix<complr4> TMP_M = new matrix(wintmp,MASTER); // construct as part
			  matrix<complr4> TMP_S = new matrix(wintmp,SLAVE); // construct as part
			  switch (filtrangeinput.oformatflag)
			{
			case FORMATCR4:
			  {
			...
			case FORMATCI2:
			  {
				matrix TMP = new matrix(1,TMP_M.pixels());
				for (uint ii =0; ii<TMP.pixels(); ++ii)
	  ...
	  of_m.close();
	  of_s.close();
	  INFO << "Mean frequency shift over " << totalblocks << "blocks = " << (totaldeltaf/1e6)/totalblocks << " MHz.";
	  INFO.print();
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  if (filtrangeinput.oformatflag==FORMATCI2)
		scratchlogfile << "complex short int";
	  scratchlogfile << "\nOutput file filtered slave (format): \t\t\t" << filtrangeinput.foslave << " ";
	  if (filtrangeinput.oformatflag==FORMATCR4)
		scratchlogfile << "complex r4";
	  if (filtrangeinput.oformatflag==FORMATCI2)
		scratchlogfile << "complex short int";
	  scratchlogfile << "\nmean shift: \t\t\t" << (totaldeltaf/1e6)/totalblocks << " MHz." << "\n*******************************************************************\n";
	  scratchlogfile.close();
...
//***************************************************************
	...
	public static void phasefilter(input_gen generalinput, productinfo interferogram, input_filtphase filtphaseinput)
	{
	  TRACE_FUNCTION("phasefilter (BK 24-Oct-2000)")

		// ====== Handle input ======
		boolean doexternalfile = false;
	  if (specified(filtphaseinput.fifiltphase))
		doexternalfile = true;
	  String infile = new String(new char[EIGHTY]); // file 2b filtered
	  infile = interferogram.file;
	  int32 numlinesinput = interferogram.win.lines()/interferogram.multilookL;
	  int32 numpixelsinput = interferogram.win.pixels()/interferogram.multilookP;
	  if (doexternalfile) // overwrite default file
		{
		  numlinesinput = filtphaseinput.finumlines;
		  infile = filtphaseinput.fifiltphase;
		  ifstream ifile;
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
		  ...
		  final streamoff sizefile = ifile.tellg(); // opened ate, [MA] 64 bit pointer on a 32-bit system
		  ifile.close();
	  ...
	  INFO << "phasefilter: inputfile: (#l,#p): " << infile << " (" << numlinesinput << "," << numpixelsinput << ")";
	  INFO.print();

	  // ______ Set variables ______
	  final real8 ALPHA = filtphaseinput.alpha; // 0<a<1;
	  final int32 SIZE = filtphaseinput.blocksize; // power of 2
	  final int32 OVERLAP = filtphaseinput.overlap; // half of the overlap
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  final int32 numout = SIZE-(2 *OVERLAP); // number of output lines
	  ...
	  boolean lastbuffer = false; // not yet, just starting...
	  ...
	  matrix<complr4> CINT = new matrix(SIZE,numpixelsinput); // type must be cr4!

	  int32 lineswritten = OVERLAP; // first buffer extra OVERLAP lines
	  final int32 numbuffers = numlinesinput/numout; // approx?
	  int32 tenpercent = (numbuffers/10)+.5; // round
	  if (tenpercent ==0)
		  tenpercent = 1000;
	  ...
	  for (int32 buffer =1; buffer<1e6; ++buffer)
		  ...
		  if (cintlinehi >= numlinesinput-1) // -1 since in matrix system
		{
		  ...
		  final int32 lines2bwritten = numlinesinput-lineswritten;
		  ...
		  final window windummy = new window(0,0,0,0); // no offset in readfile
		  final window wincint = new window(cintlinelo,cintlinehi,0,numpixelsinput-1);
		  ...
		  matrix<complr4> FILTERED = goldstein(CINT, ALPHA, OVERLAP, filtphaseinput.kernel);

		  // ______ Write filtered data ______
		  final window winfiltered = new window(filteredlinelo,filteredlinehi,0,numpixelsinput-1);
	  ...
	  // ====== Write results to file ======
	  ofstream scratchlogfile = new ofstream("scratchlogfiltphase", ios.out | ios.trunc);
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  // ______ updateproductinfo greps info from this file ______
	  ofstream scratchresfile = new ofstream("scratchresfiltphase", ios.out | ios.trunc);
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  bk_assert(scratchresfile,"scratchresfiltphase",__FILE__,__LINE__);
	  scratchresfile << "\n\n*******************************************************************" << "\n*_Start_" << processcontrol[pr_i_filtphase] << "\n*******************************************************************" << "\nMethod_phasefilt: goldstein: size, alpha, overlap: \t" << SIZE << " " << ALPHA << " " << OVERLAP << "\n1D Smoothing kernel for |spectrum|:   \t";
...
//***************************************************************
	...
	public static matrix<complr4> goldstein(matrix<complr4> CINT, real4 ALPHA, int32 OVERLAP, matrix<real4> smoothkernel)
	{
	...
	//#else
	  ...
	  final int32 SIZE = CINT.lines();
	  final int32 NPIX = CINT.pixels();
	  matrix<complr4> FILTERED = new matrix(SIZE,NPIX); // output

	  // ______ Get block from buffer ______
	  final int32 numout = SIZE-(2 *OVERLAP); // number of output pixels
	  ...
	  final int32 SMOOTH = int32(smoothkernel.pixels())/2; // half block size, odd kernel
	  final boolean dosmooth = (SMOOTH ==0) ? false : true;
	  ...
	  if (dosmooth ==true)
		{
		  matrix<complr4> kernel = new matrix(1,SIZE); // init to zeros
	  ...
	  for (;;) //forever
		  ...
		  if (cintpixhi>=NPIX-1) // check if we are doing the last block
		{
		  ...
		  matrix<complr4> BLOCK = new matrix(wincint,CINT);
	...
	//#if ! CHECKINDEXONLY
		...
		AMPLITUDE = smooth(AMPLITUDE, KERNEL2D);
		  ...
		  final real4 maxamplitude = max(AMPLITUDE);
...
//***************************************************************
	...
	public static matrix<real4> smooth(matrix<real4> A, int32 N)
	{
	//C++ TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	//#if __DEBUGMAT2
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	//#if SPACEDOMAIN
	  final int32 L = A.lines();
	  final int32 P = A.pixels();
	  matrix<real4> SMOOTH = new matrix(L,P); // init to zero...
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register real8 sum = 0.;
	  real8 sum = 0.;
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register int32 indexii;
	  int32 indexii;
	  final real8 Nsmooth = (2 *N+1)*(2 *N+1);
	...
	//#if SPECTRALDOMAIN
	  ...
	  matrix<complr4> kernel = new matrix(1,L); // init to zeros
...
//***************************************************************
	...
	public static matrix<real4> smooth(matrix<real4> A, matrix<complr4> KERNEL2D)
	{
	//C++ TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	//#if __DEBUGMAT2
...
//***************************************************************
	...
	public static void spatialphasefilt(input_gen generalinput, productinfo interferogram, input_filtphase filtphaseinput)
	{
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
		  ...
		  final streamoff sizefile = ifile.tellg(); // opened ate, MA 64 bit pointer
		  ifile.close();
		  numpixelsinput = sizefile/sizeof(complr4)/numlinesinput; // floor
	  ...
	  if (generalinput.memory<100e6) // check memory size 512
		  SIZE/=2;
	  if (generalinput.memory<20e6) // check memory size 256
		  SIZE/=2;
	  while (numlinesinput < SIZE)
		  SIZE/=2;
	  while (numpixelsinput < SIZE)
		  SIZE/=2;
	  ...
	  if (!specified(filtphaseinput.fikernel2d))
		{
		  ...
		  OVERLAP_LINES = int32(Math.floor(KERNELSIZE_LINES/2.));
		  OVERLAP_PIXELS = OVERLAP_LINES;

		  // ______ 1d kernel function ______
		  matrix<complr4> kernel = new matrix(1,SIZE); // init to zeros
		  for (register int32 ii =-OVERLAP_LINES; ii<=OVERLAP_LINES; ++ii)
		kernel(0,(ii+SIZE)%SIZE) = complr4(filtphaseinput.kernel(0,ii+OVERLAP_LINES),0.0); // BK 07-Apr-2003
	  ...
	  else // use filename and blocksize
		{
		  ...
		  ifstream kernel2d = new ifstream(filtphaseinput.fikernel2d, ios.in);
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
		  bk_assert(kernel2d,filtphaseinput.fikernel2d,__FILE__,__LINE__);
		  real4 scalefactor;
		  kernel2d >> KERNELSIZE_LINES >> KERNELSIZE_PIXELS >> scalefactor;
		  if (Math.abs(scalefactor)<=EPS)
			  scalefactor =1.;
		  INFO << "kernel2d: file: lines: rows: scale: " << filtphaseinput.fikernel2d << " " << KERNELSIZE_LINES << " " << KERNELSIZE_PIXELS << " " << scalefactor;
		  INFO.print();
		...
		{
		  real4 tmpvalue;
		  String dummyline = new String(new char[10 *ONE27]); // take care of very large kernel
		  kernel2d.getline(dummyline,10 *ONE27,'\n');
		  final int32 indexii = (ii+SIZE)%SIZE;
		  for (int32 jj =-OVERLAP_PIXELS; jj<=OVERLAP_PIXELS; ++jj)
			{
			  final int32 indexjj = (jj+SIZE)%SIZE;
			  kernel2d >> tmpvalue;
	  ...
	  INFO.print();
	  INFO << "kernelsize: (" << KERNELSIZE_LINES << ", " << KERNELSIZE_PIXELS << ")";
	  INFO.print();
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  // ______ initialize indices ______
	  final int32 numout = SIZE-(2 *OVERLAP_LINES); // number of output lines per buffer
	  int32 cintlinelo = 0; // index in CINT to get 1st buffer
	  int32 cintlinehi = SIZE-1; // index in CINT to get 1st buffer
	  int32 filteredlinelo = 0; // index in FILTERED to write (1st)
...
//***************************************************************
	...
	public static matrix<complr4> convbuffer(matrix<complr4> CINT, matrix<complr4> KERNEL2D, int32 OVERLAP)
	{
	...
	//#else
	  ...
	  final int32 SIZE = CINT.lines();
	  final int32 NPIX = CINT.pixels();
	  matrix<complr4> FILTERED = new matrix(SIZE,NPIX); // allocate output (==0)

	  // ______ Get block from buffer ______
	  final int32 numout = SIZE-(2 *OVERLAP); // number of output pixels per block
	  int32 cintpixlo = 0; // index in CINT to get 1st block
	  int32 cintpixhi = SIZE-1; // index in CINT to get 1st block
	  //int32 outblockpixlo = 0;                    // index in BLOCK (only 1st block)
	...
	//#if ! CHECKINDEXONLY
		  ...
		  ifft2d(BLOCK); // conv. in space domain FILTERED.setdata(winfiltered,BLOCK,winblock);
	//#else
		  FILTERED.setdata(winfiltered,BLOCK,winblock);
	//#endif // check index blocks
		  // ______ Set correct part that is filtered in output matrix ______
...
//***************************************************************
	...
	public static void phasefilterspectral(input_gen generalinput, productinfo interferogram, input_filtphase filtphaseinput)
	{
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  final int32 HBS_L = (KERNELSIZE_LINES/2);
	  final int32 HBS_P = (KERNELSIZE_PIXELS/2);
	  final int32 EXTRA_L = (iseven(KERNELSIZE_LINES)) ? 1:0; // 1 less to fill
	  final int32 EXTRA_P = (iseven(KERNELSIZE_PIXELS)) ? 1:0; // 1 less to fill
	  matrix<real4> KERNEL2D = new matrix(SIZE,SIZE); // allocate THE matrix
	  for (int32 ii =-HBS_L+EXTRA_L; ii<=HBS_L; ++ii)
		{
		  String dummyline = new String(new char[10 *ONE27]); // prevent very large kernels
		  kernel2d.getline(dummyline,10 *ONE27,'\n');
		  final int32 indexii = (ii+SIZE)%SIZE;
	  ...
	  final int32 numbuffers = numlinesinput/SIZE; // approx for large mem?
	  int32 tenpercent = (numbuffers/10)+.5; // round
	  if (tenpercent ==0) // avoid error: x%0
		  tenpercent = 1000;
	  PROGRESS.print("filtphase:  0%");
	  ...
	  for (int32 buffer =1; buffer<1e6; ++buffer)
		  ...
		  if (cintlinehi >= numlinesinput-1) // -1 since in matrix system
		{
		  ...
		  // ______ Filter data in buffer of BLOCKSIZE lines ______
		  // ______ output has same size, but write only part to disk ______
		  final matrix<complr4> FILTERED = spectralfilt(CINT, KERNEL2D, OVERLAP);
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  bk_assert(scratchresfile,"scratchresfiltphase",__FILE__,__LINE__);
		// TODO
	  scratchresfile << "\n\n*******************************************************************" << "\n*_Start_" << processcontrol[pr_i_filtphase] << "\n*******************************************************************" << "\nMethod_phasefilt: spectral: size, overlap, kernelfile: \t" << SIZE << " " << OVERLAP << " " << filtphaseinput.fikernel2d << "\nInput_file:                           \t" << infile << "\nData_output_file:                     \t" << filtphaseinput.fofiltphase << "\nData_output_format:                   \t" << "complex_real4" << "\nFirst_line (w.r.t. original_master):  \t" << interferogram.win.linelo << "\nLast_line (w.r.t. original_master):   \t" << interferogram.win.linehi << "\nFirst_pixel (w.r.t. original_master): \t" << interferogram.win.pixlo << "\nLast_pixel (w.r.t. original_master):  \t" << interferogram.win.pixhi << "\nMultilookfactor_azimuth_direction:    \t" << interferogram.multilookL << "\nMultilookfactor_range_direction:      \t" << interferogram.multilookP << "\nNumber of lines (multilooked):        \t" << numlinesinput << "\nNumber of pixels (multilooked):       \t" << numpixelsinput << "\n*******************************************************************" << "\n* End_" << processcontrol[pr_i_filtphase] << "_NORMAL" << "\n*******************************************************************\n";
	  scratchresfile.close();
	  // ====== Tidy up ======
...
//***************************************************************
	...
	public static matrix<complr4> spectralfilt(matrix<complr4> CINT, matrix<real4> KERNEL2D, int32 OVERLAP)
	{
	...
	//#if WIN32
		  ...
		  BLOCK = timesCxR(BLOCK,temp); // Jia ifft2d(BLOCK);
	//#else
		  BLOCK *= KERNEL2D; // the filter... ifft2d(BLOCK);
	//#endif
		  // ______ Set correct part that is filtered in output matrix ______
		  FILTERED.setdata(winfiltered,BLOCK,winblock);
...
//***************************************************************
	...
	// ====== azimuth filter ======
	public static void azimuthfilter(input_gen generalinput, input_filtazi filtaziinput, RefObject<slcimage> master, RefObject<slcimage> slave)
	{
	  TRACE_FUNCTION("azimuthfilter (BK 01-Nov-2000)")
		// ====== Handle input ======
		String infile = new String(new char[EIGHTY]); // file 2b filtered
	  infile = master.argvalue.file;
	  int32 numlinesinput = master.argvalue.currentwindow.lines();
	  int32 numpixelsinput = master.argvalue.currentwindow.pixels();
	  INFO << "azimuthfilter: inputfile: (#l,#p): " << infile << " (" << numlinesinput << "," << numpixelsinput << ")";
	  INFO.print();

	  // ______ Set variables ______
	  final int32 SIZE = filtaziinput.fftlength; // power of 2
	  final int32 OVERLAP = filtaziinput.overlap; // half of the overlap
	  final real8 HAMMING = filtaziinput.hammingalpha;
	  ...
	  final real8 max_fdc_change = 0.30 *Math.abs(master.argvalue.prf - master.argvalue.abw); // 100 Hz or so for ERS
	  final real8 master_fdc_p0 = master.argvalue.pix2fdc(master.argvalue.currentwindow.pixlo);
	  final real8 master_fdc_p05 = master.argvalue.pix2fdc((master.argvalue.currentwindow.pixhi-master.argvalue.currentwindow.pixlo)/2);
	  final real8 master_fdc_pN = master.argvalue.pix2fdc(master.argvalue.currentwindow.pixhi);
	  final real8 slave_fdc_p0 = slave.argvalue.pix2fdc(slave.argvalue.currentwindow.pixlo);
	  final real8 slave_fdc_p05 = slave.argvalue.pix2fdc((slave.argvalue.currentwindow.pixhi-slave.argvalue.currentwindow.pixlo)/2);
	  final real8 slave_fdc_pN = slave.argvalue.pix2fdc(slave.argvalue.currentwindow.pixhi);
	  final real8 master_max_dfdc = max(Math.abs(master_fdc_p0-master_fdc_p05),Math.abs(master_fdc_p0-master_fdc_pN));
	  final real8 slave_max_dfdc = max(Math.abs(slave_fdc_p0-slave_fdc_p05),Math.abs(slave_fdc_p0-slave_fdc_pN));
	  INFO << "Master: fDC Variation [Hz] = " << master_max_dfdc;
	  INFO.print();
	  INFO << "Slave: fDC Variation [Hz]  = " << slave_max_dfdc;
	  INFO.print();
	  if (master_max_dfdc<max_fdc_change && slave_max_dfdc <max_fdc_change)
		{
		  ...
		  master.argvalue.f_DC_a0 = master_fdc_p05;
		  master.argvalue.f_DC_a1 = 0.0;
		  master.argvalue.f_DC_a2 = 0.0;
		  INFO << "Master: constant fDC [Hz] = " << master.argvalue.f_DC_a0;
		  INFO.print();
		  slave.argvalue.f_DC_a0 = slave_fdc_p05;
		  slave.argvalue.f_DC_a1 = 0.0;
		  slave.argvalue.f_DC_a2 = 0.0;
		  INFO << "Slave: constant fDC [Hz] = " << slave.argvalue.f_DC_a0;
		  INFO.print();
		}

	  // ====== Find out if same filter for all columns can be used ======
	  boolean samefd0 = false; // assume NOT same filter
	  if (Math.abs(master.argvalue.f_DC_a1)<EPS && Math.abs(master.argvalue.f_DC_a2)<EPS && Math.abs(slave.argvalue.f_DC_a1)<EPS && Math.abs(slave.argvalue.f_DC_a2)<EPS) // true for eSARp'ed processed raw data.
		samefd0 = true;
	  else
	  ...
	  if (samefd0 ==true)
		{
		  ...
		  final real8 PRF = master.argvalue.prf; // pulse repetition freq. [Hz]
		  final real8 ABW = master.argvalue.abw; // azimuth band width [Hz]
		  final real8 fDC_m = master.argvalue.f_DC_a0; // zero doppler freq. [Hz]
		  final real8 fDC_s = slave.argvalue.f_DC_a0; // zero doppler freq. [Hz]
		  final real8 fDC_mean = 0.5*(fDC_m+fDC_s); // mean doppler centroid freq.
		  final real8 ABW_new = max(1.0, 2.0*(0.5 *ABW-Math.abs(fDC_m-fDC_mean))); // new bandwidth>1.0
		  INFO << "New Azimuth Bandwidth: " << ABW_new << " [Hz]";
		  INFO.print();
		  INFO << "New central frequency: " << fDC_mean << " [Hz]";
		  INFO.print();

		  final real4 deltaf = PRF/real4(SIZE);
		  final real4 fr = -PRF/2.0;
		  matrix<real4> freqaxis = new matrix(1,SIZE);
		  ...
		  if (dohamming)
		{
		  ...
		  int32 myshift = ((SIZE *fDC_m/PRF) + 0.5); // round
	...
	//#endif
		...
		{
		  int32 myshift = ((SIZE *fDC_mean/PRF) + 0.5); // round
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  int32 percent = 10;
	  final int32 numbuffers = (numlinesinput+OVERLAP)/numout; // approx?
	  int32 tenpercent = rint(numbuffers/10.0); // round
	  if (tenpercent ==0) // avoid error: x%0
		  tenpercent = 1000;
	  PROGRESS.print("FILTAZI:  0%");

	  // ====== Start filter loop buffers of BLOCKSIZE ======
	  matrix<complr4> SLCIMAGE = new matrix(SIZE,numpixelsinput);
	  for (int32 buffer =1; buffer<1e6; ++buffer)
		{
		  // ______ Check if this will be the last buffer ______
		  if (slclinehi >= numlinesinput-1) // -1 since in matrix system
		{
		  ...
		  final window windummy = new window(0,0,0,0); // no offset in readfile
		  final window winslc = new window(slclinelo,slclinehi,0,numpixelsinput-1);
		  switch (master.argvalue.formatflag)
		{
		case FORMATCI2:
		  {
		  ...
		  else
		{
		  ...
		  FILTERED = blockazifilt(SLCIMAGE, master.argvalue, slave.argvalue, HAMMING);
		}
		...
		case FORMATCI2:
		  {
			matrix TMP = new matrix(FILTERED.lines(),FILTERED.pixels());
	  ...
	  // ====== Write results to file ======
	  ofstream scratchlogfile = new ofstream("scratchlogfiltazi", ios.out | ios.trunc);
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  // ______ updateproductinfo greps info from this file ______
	  ofstream scratchresfile = new ofstream("scratchresfiltazi", ios.out | ios.trunc);
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  if (filtaziinput.oformatflag==FORMATCI2)
		  scratchresfile << "complex_short";
	  scratchresfile << "\nFirst_line (w.r.t. original_image):   \t" << master.argvalue.currentwindow.linelo << "\nLast_line (w.r.t. original_image):    \t" << master.argvalue.currentwindow.linehi << "\nFirst_pixel (w.r.t. original_image):  \t" << master.argvalue.currentwindow.pixlo << "\nLast_pixel (w.r.t. original_image):   \t" << master.argvalue.currentwindow.pixhi << "\n*******************************************************************" << "\n* End_" << processcontrol[pr_m_filtazi] << "_NORMAL" << "\n*******************************************************************\n";
	  scratchresfile.close();
	  // ====== Tidy up ======
	} // END azimuthfilter
...
//***************************************************************
	...
	public static matrix<complr4> blockazifilt(matrix<complr4> SLCIMAGE, slcimage master, slcimage slave, real8 HAMMING)
	{
	  TRACE_FUNCTION("blockazifilt (BK 06-Nov-2000)")
		final uint SIZE = SLCIMAGE.lines(); // fftlength
	  final uint NCOLS = SLCIMAGE.pixels(); // width
	  ...
	  matrix<real8> xaxis = new matrix(1,master.currentwindow.pixels()); // lying
	...
	//#endif
	  ...
	  final boolean dohamming = (HAMMING<0.9999) ? true : false;
	  final real8 PRF = master.prf; // pulse repetition freq. [Hz]
	  final real8 ABW = master.abw; // azimuth band width [Hz]
	  ...
	  matrix<real4> FILTERMAT = new matrix(SIZE,NCOLS); // i.e., THE filter
	  for (uint i =0; i<NCOLS; ++i)
		{
		  final real8 fDC_m = FDC_M(0,i); // zero doppler freq. [Hz]
		  final real8 fDC_s = FDC_S(0,i); // zero doppler freq. [Hz]
		  final real8 fDC_mean = 0.5*(fDC_m+fDC_s); // mean doppler centroid freq.
		  final real8 ABW_new = max(1.0, 2.0*(0.5 *ABW-Math.abs(fDC_m-fDC_mean))); // new bandwidth > 1.0
		  if (dohamming)
		{
		  ...
		  // ______ Shift this circular by myshift pixels ______
		  int32 myshift = rint((SIZE *fDC_m/PRF)); // round
		  wshift(inversehamming,-myshift); // center at fDC_m
		...
		{
		  int32 myshift = rint((SIZE *fDC_mean/PRF)); // round
		  FILTER = myrect(freqaxis/real4(ABW_new)); // fftshifted
		  wshift(FILTER,-myshift); // center at fDC_mean
		}
		  ifftshift(FILTER); // fftsh works on data!
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	//#if WIN32
	  FILTERED = timesCxR(FILTERED,FILTERMAT); // Jia ifft(FILTERED,1); // ifft foreach column
	//#else
	  FILTERED *= FILTERMAT; // filter each column ifft(FILTERED,1); // ifft foreach column
	//#endif
	  return FILTERED;
	} // END blockazifilt
