public class GlobalMembersGeocode
{
...
// ***************************************************************
	...
	public static void slant2hschwabisch(input_gen generalinput, input_slant2h slant2hinput, input_ell ellips, slcimage master, slcimage slave, productinfo unwrappedinterf, RefObject<orbit> masterorbit, RefObject<orbit> slaveorbit)
	  {
	  ...
	  final real8 m_minpi4cdivlambda = (-4.*PI *SOL)/master.wavelength;
	  final real8 s_minpi4cdivlambda = (-4.*PI *SOL)/slave.wavelength;

	  final int32 Npoints = slant2hinput.Npoints; // where ref.phase is evaluated
	  final int32 DEGREE1D = slant2hinput.degree1d; // only possible now.
	  final int32 DEGREE2D = slant2hinput.degree2d;
	  final int32 Nheights = slant2hinput.Nheights;

	  final int32 MAXHEIGHT = 5000; // max hei for ref.phase
	  final int32 TEN = 10; // used in pointer
	  if (DEGREE1D - 1 > TEN)
		{
	  ...
	  final int32 HEIGHTSTEP = MAXHEIGHT / (Nheights - 1); // heights to eval ref.phase
	  ...
	  matrix<real8> PHASE = new matrix(Npoints,Nheights); // pseudo-observation
...
//ORIGINAL LINE: register int32 numheight;
	  int32 numheight;
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register int32 i,j,k,l,index;
	  int32 i;
	  int32 j;
	  int32 k;
	  int32 l;
	  int32 index;
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register int32 alfa;
	  int32 alfa;
	  for (numheight =0; numheight<Nheights; numheight++)
		{
		cn pospoint;
		final int32 HEIGHT = numheight * HEIGHTSTEP;
		final input_ell ELLIPS = new input_ell(ellips.a+HEIGHT, ellips.b+HEIGHT);
		...
		for (i =0;i<Npoints;i++)
		  ...
		  lp2xyz(line,pixel,ELLIPS,master,masterorbit.argvalue, pospoint,MAXITER,CRITERPOS);

		  // ______ Compute xyz of slave satelite in orbit_slave from P ______
		  real8 s_tazi; // returned
		  real8 s_trange; // returned, unused?
		  xyz2t(s_tazi,s_trange,slave, slaveorbit.argvalue, pospoint,MAXITER,CRITERTIM);
		  ...
		  PHASE(i,numheight) = m_minpi4cdivlambda *m_trange- s_minpi4cdivlambda *s_trange;
		  }
		}
	...
	// ====== STEP 2 ======
	  ...
	  matrix<real8> DESIGN = new matrix(Nheights,DEGREE1D+1); // design matrix
	  matrix<real8> ALPHAS = new matrix(Npoints,DEGREE1D+1); // pseudo-observation
	  matrix<real8> HEI = new matrix(Nheights,1);
	  for (i =0; i<Nheights; i++)
		HEI(i,0) = i *HEIGHTSTEP; // 0, .., 5000

	  // ______ normalize data to [0,1] ______
	  final real8 minphi = min(PHASE);
	  final real8 maxphi = max(PHASE);
	  ...
	  for (i =0; i<Npoints; i++) // solve system for all points
		...
		for (j =0; j<Nheights; j++)
		  for (k =0; k<=DEGREE1D; k++)
			DESIGN(j,k) = Math.pow(PHASE(i,j),real8(k)); // PHASE is normalized
		...
		final real8 maxdev = max(Math.abs(N *Qx_hat-eye(real8(Qx_hat.lines()))));
	...
	// ====== STEP 3 ======
	  ...
	  final int32 Nunk = Ncoeffs(DEGREE2D); // Number of unknowns
	  ...
	  final real8 minL = min(Position.getcolumn(0));
	  final real8 maxL = max(Position.getcolumn(0));
	  final real8 minP = min(Position.getcolumn(1));
	  final real8 maxP = max(Position.getcolumn(1));
	  for (i =0; i<Npoints; i++)
		{
		// ______ normalize coordinates ______
		final real8 posL = normalize(real8(Position(i,0)),minL,maxL);
		final real8 posP = normalize(real8(Position(i,1)),minP,maxP);
	...
	/////  solvechol(Qx_hat,rhs);        // Estimate of unknowns (betas) in rhs, NOT OK!
	  ...
	  if (maxdev > 0.01)
		{
		WARNING << "slant2h: possibly wrong solution. deviation from unity AtA*inv(AtA) = " << maxdev << " > 0.01";
		WARNING.print();
		}
	...
	// ====== STEP 4 ======
	  ...
	  final real8 multiL = unwrappedinterf.multilookL;
	  final real8 multiP = unwrappedinterf.multilookP;


	  // ______ Number of lines/pixels of multilooked unwrapped interferogram ______
	  final int32 mllines = Math.floor(real8(unwrappedinterf.win.linehi-unwrappedinterf.win.linelo+1) / multiL);
	  final int32 mlpixels = Math.floor(real8(unwrappedinterf.win.pixhi-unwrappedinterf.win.pixlo+1) / multiP);


	  // ______ Line/pixel of first point in original master coordinates ______
	  final real8 veryfirstline = real8(unwrappedinterf.win.linelo) + (real8(multiL) - 1.) / 2.;
	  final real8 firstpixel = real8(unwrappedinterf.win.pixlo) + (real8(multiP) - 1.) / 2.;


	  // ______ Constant axis of pixel coordinates ______
	  matrix<real4> p_axis = new matrix(mlpixels,1);
	  ...
	  final int32 NUMMAT = 1 + (1 + DEGREE1D); // number of heavy matrices
	  int32 bufferlines = generalinput.memory / (NUMMAT * (mlpixels * sizeof(real4)));
	  if (bufferlines > mllines) // whole image fits in BUFFER
		bufferlines = mllines;

	  final int32 FULLBUFFERS = mllines / bufferlines;
	  final int32 RESTLINES = mllines % bufferlines;
	  final int32 EXTRABUFFER = RESTLINES != 0 ? 1 : 0;

	  // ______ Window to be read into BUFFER from file in multilooked system ______
	  final uint dummy = 999999; // large to force error if not ok
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  for (register int32 buffer =1; buffer<=FULLBUFFERS+EXTRABUFFER; buffer++)
		...
		PROGRESS << "SLANT2H: " << 100*(buffer-1)/(FULLBUFFERS+EXTRABUFFER) << "%";
		PROGRESS.print();

		// ______ In original master coordinate sytem ______
		final real8 firstline = veryfirstline + (buffer-1) * bufferlines * multiL;
		...
		matrix<real4>[] pntALPHA = new matrix[TEN];
		for (k =0; k<=DEGREE1D; k++)
		  {
		  matrix<real8> beta = new matrix(Ncoeffs(DEGREE2D),1);
		...
		matrix<real8> coeff_thispoint = new matrix(DEGREE1D+1,1);
		for (uint line =0; line<BUFFER.lines(); line++)
		  ...
		  for (uint pixel =0; pixel<BUFFER.pixels(); pixel++)
			...
			if (BUFFER(line,pixel) != NaN) // else leave NaN
			  ...
			  BUFFER(line,pixel) = polyval1d(normalize(BUFFER(line,pixel),minphi,maxphi), coeff_thispoint);
			  }
			}
		  }
	...
	// ====== Write to result files ======
	  ofstream scratchlogfile = new ofstream("scratchlogslant2h", ios.out | ios.trunc);
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  ofstream scratchresfile = new ofstream("scratchresslant2h", ios.out | ios.trunc);
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
...
// ***************************************************************
	...
	public static void slant2hambiguity(input_gen generalinput, input_slant2h slant2hinput, input_ell ellips, slcimage master, slcimage slave, productinfo unwrappedinterf, RefObject<orbit> masterorbit, RefObject<orbit> slaveorbit, BASELINE baseline)
	  {
	  TRACE_FUNCTION("slant2hambiguity (BK 07-Jul-1999)")

	  final int32 MAXITER = 10; // iterations for lp2xyz
	  final real8 CRITERPOS = 1e-6; // stop criterium for lp2xyz
	  final real8 CRITERTIM = 1e-10; // stop criterium for lp2xyz

	  final int32 MAXITERHERE = 4; // iterations for h
	  final real8 CRITERHERE = 0.05; // m (delta h in iterations)
	  ...
	  final int32 NUMMAT = 3; // number of large matrices BUFFER PHI LAMBDA
	  int32 bufferlines = generalinput.memory / (NUMMAT * (mlpixels * sizeof(real4)));
	  if (bufferlines > mllines) // whole image fits in BUFFER
		bufferlines = mllines;
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  matrix<real8> observations = new matrix(3,1); // setofeq
	  matrix<real8> partials = new matrix(3,3); // partial derivatives
	  matrix<real8> solution = new matrix(3,1); // solution of setofeq.
	  ...
	  for (register int32 buffer =1; buffer<=FULLBUFFERS+EXTRABUFFER; ++buffer)
		...
		matrix<real4> BUFFER = unwrappedinterf.readphase(bufferwin);
		matrix<real4> PHI = new matrix(BUFFER.lines(),BUFFER.pixels()); // geocoded
		matrix<real4> LAMBDA = new matrix(BUFFER.lines(),BUFFER.pixels()); // geocoded
...
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register real8 line = firstline - multiL;
		real8 line = firstline - multiL; // in master coordinate system
...
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register real8 pixel = firstpixel - multiP;
		  real8 pixel = firstpixel - multiP; // in master coordinate system
		  ...
		  Mdot = masterorbit.argvalue.getxyzdot(m_tazi);
		  final real8 norm2M = M.norm2(); // (sqr) constant per line
		  ...
		  boolean lineokunwrapped = false;
		  for (uint t1 =0; t1<BUFFER.pixels(); ++t1)
			if (BUFFER(i,t1) != NaN)
				lineokunwrapped = true;
	...
	boolean DO_NEW_METHOD =false; // no time to test this, methdo seems wrong??
	if (DO_NEW_METHOD ==false) // old method, not using BASELINE class:
	{
			  ...
			  lp2xyz(line,temppixel, ELLIPS,master,masterorbit.argvalue, P,MAXITER,CRITERPOS); // P returned
			  ...
			  final real8 B = M.dist(S); // abs. value
			  Bpar = P.dist(M) - P.dist(S); // sign ok


			  // ______ Bperp>0 if (MP>SP) then S is to the right of slant line
			  costheta = ((-(P.norm2()) + norm2M + sqr(master.pix2range(temppixel))) / (2 *Math.sqrt(norm2M)*master.pix2range(temppixel)));
			  sintheta = Math.sqrt(1-sqr(costheta)); // cos^2 + sin^2 = 1

			  final cn r2 = S.min(P); // vector; to find sign
			  final real8 costheta2 = M.in(r2) / (M.norm()*r2.norm());
			  Bperp = (costheta < costheta2) ? Math.sqrt(sqr(B)-sqr(Bpar)) : -Math.sqrt(sqr(B)-sqr(Bpar));
	}
	else
	{
	...
	costheta = Math.cos(baseline.get_theta_inc(line,temppixel,currentheight));
	sintheta = Math.sqrt(1.0-sqr(costheta));
	inc_angle = baseline.get_theta_inc(line,temppixel,currentheight); //KKM
	}

			  // --- Update height ---
			  lastheight = currentheight;
			  final real8 m_tr = master.pix2tr(temppixel);
			  final real8 tempr1 = SOL *m_tr;
			  currentheight = (-master.wavelength *tempr1 *Math.sin(inc_angle)*tempphase)/ (4.*PI *Bperp); //KKM added this


			  // ______ Check convergence ______
			  if (Math.abs(currentheight-lastheight) < CRITERHERE)
			...
			if (j >= MAXITERHERE)
			  {
			  WARNING << "slant2hambiguity: maxiter reached. " << "MAXITER: " << MAXITERHERE << "CRITER: " << CRITERHERE << "m " << "last correction: " << currentheight-lastheight;
			  WARNING.print();
			  }
		  ...
		  // BK 09-Aug-2000
		  final real8 Bhor = Bperp *costheta + Bpar *sintheta;
		  final real8 Bver = Bperp *sintheta - Bpar *costheta;
	...
	// ====== Start loop over pixels ======
		  for (uint j =0; j<BUFFER.pixels(); j++)
			...
			if (BUFFER(i,j) == NaN) // leave NaN in buffer
			  ...
			  final real8 m_trange = master.pix2tr(pixel);
			  final real8 normr1 = SOL *m_trange;
			  final real8 partnumerator = norm2M + sqr(normr1);
			  final real8 denominator = 2.0 *Math.sqrt(norm2M)*normr1;
			  final real8 m_lamr1phidiv4pi = master.wavelength *normr1 *BUFFER(i,j) / (4.0 *PI);

			  // ______ Iteratively solve for height ______
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register int32 iteration;
			  int32 iteration;
			  for (iteration =0; iteration<=MAXITERHERE; ++iteration)
				...
				ELLIPS.a = ellips.a + currentheight; //next height
				ELLIPS.b = ellips.b + currentheight; //next height
				lp2xyz(line,pixel, ELLIPS,master,masterorbit.argvalue, P,MAXITER,CRITERPOS); // P returned
				...
				currentheight = (-m_lamr1phidiv4pi * Math.sin(inc_angle)) / (Bhor *costheta + Bver *sintheta); //KKM
...
// ***************************************************************
	...
	public static void slant2hrodriguez(input_gen generalinput, input_slant2h slant2hinput, input_ell ellips, slcimage master, slcimage slave, productinfo unwrappedinterf, matrix<real8> coeff_flatearth, RefObject<orbit> masterorbit, RefObject<orbit> slaveorbit, BASELINE baseline)
	  {
	  ...
	  final int32 degreecfe = degree(coeff_flatearth.size());
	  ...
	  final real8 multiL = unwrappedinterf.multilookL;
	  final real8 multiP = unwrappedinterf.multilookP;
	  final real8 m_pi4divlambda = (4.*PI)/master.wavelength;
	  ...
	  // ====== Compute number of buffers required ======
	  final int32 NUMMAT = 1; // number of large matrices to determine size of matrices
	  int32 bufferlines = generalinput.memory / (NUMMAT * (mlpixels * sizeof(real4)));
	  if (bufferlines > mllines) // whole image fits in BUFFER
		bufferlines = mllines;
...
//ORIGINAL LINE: register real8 pixel = firstpixel - multiP;
		  ...
		  // ______ only dependent on line ______
		  final real8 m_tazi = master.line2ta(line);
		  cn M = masterorbit.argvalue.getxyz(m_tazi);
		  cn P; // coordinates of point on earth

		  // ______ compute baseline for point on ellips (h) ______
		  // ______ Compute P(x,y,z) only to get baseline and alpha for this line ______
		  final real8 middlepixel = pixel+(.5 *BUFFER.pixels()*multiP);
		  lp2xyz(line,middlepixel, ellips,master,masterorbit.argvalue, P,MAXITER,CRITERPOS); // P returned
		  ...
		  real8 s_trange; // returned, unused?
		  xyz2t(s_tazi,s_trange,slave, slaveorbit.argvalue, P,MAXITER,CRITERTIM);
		  cn S = slaveorbit.argvalue.getxyz(s_tazi);
	...
	//   -> P(h2) -> S -> B -> h3 etc.
		  ...
		  // ______ theta is angle (M,M-P) ______
		  final real8 B = M.dist(S); // abs. value
		  final real8 Bpartemp = P.dist(M) - P.dist(S); // sign ok

		  // ______ if (MP>SP) then S is to the right of slant line, then B perp is positive.
		  final real8 rho1sqr = M.norm2(); // (sqr) constant per line
		  final real8 costhetatemp = ((-(P.norm2()) + rho1sqr + sqr(master.pix2range(middlepixel))) / (2 *Math.sqrt(rho1sqr)*master.pix2range(middlepixel)));
		  //sintheta = sqrt(1-sqr(costhetatemp));           // cos^2 + sin^2 = 1

		  final cn r2 = S.min(P); // vector; to find out sign
		  final real8 Bperp = (Math.acos(costhetatemp) > M.angle(r2)) ? Math.sqrt(sqr(B)-sqr(Bpartemp)) : -Math.sqrt(sqr(B)-sqr(Bpartemp));

		  final real8 alpha = Math.acos(costhetatemp) - Math.atan2(Bpartemp,Bperp);
		  ...
		  final real8 rho1 = Math.sqrt(rho1sqr);
		  real8 satphi;
		  real8 satlambda;
		  real8 satheight;
		  ellips.xyz2ell(M, satphi,satlambda,satheight);
		  final real8 Radius = rho1 - satheight;
		  ...
		  for (uint j =0; j<BUFFER.pixels(); j++)
			...
			if (BUFFER(i,j) != NaN) // leave NaN in buffer
			  ...
			  // ______ Compute some constants (per pixel) ______
			  final real8 r1 = master.pix2range(pixel);
			  final real8 r1sqr = sqr(r1);
			  // ______ Compute reference phase to obtain Bpar ______
			  // const real8 phiref= polyval(line,pixel,coeff_flatearth,degreecfe);
			  final real8 phiref = polyval(normalize(line,minL,maxL), normalize(pixel,minP,maxP), coeff_flatearth,degreecfe);
			  //const real8 Bpar  = -(BUFFER(i,j)+phiref)/pi4divlambda;
			  // master [or slave] wavelength???
			  final real8 Bpar = -(BUFFER(i,j)+phiref)/m_pi4divlambda;

			  final real8 sinthetaminalpha = (sqr(r1)+sqr(B)-sqr(r1-Bpar))/(2 *r1 *B);
	...
	// ______ for now use theta is approx 20 degrees., (theta=1)=57 degrees
			  real8 theta = Math.asin(sinthetaminalpha)+alpha;
			  if (theta<0.0 || theta>1.0)
				theta = PI-Math.asin(sinthetaminalpha)+alpha;

			  final real8 costheta = Math.cos(theta);
			  final real8 psqr = rho1sqr+r1sqr - 2 *rho1 *r1 *costheta; // cosine law
			  final real8 p = Math.sqrt(psqr);
			  final real8 cosmu = ((-r1sqr+psqr+rho1sqr) / (2 *p *rho1)); // cosine law
			  final real8 H = rho1 - Radius *cosmu;
...
// ***************************************************************
	...
	public static void geocode(input_gen generalinput, input_geocode geocodeinput, input_ell ellips, slcimage master, productinfo heightinradarsystem, RefObject<orbit> masterorbit)
	  {
	  ...
	  // ______ Multilook factors ______
	  final real8 multiL = heightinradarsystem.multilookL;
	  final real8 multiP = heightinradarsystem.multilookP;

	  // ______ Number of lines/pixels of multilooked unwrapped interferogram ______
	  final int32 mllines = Math.floor(real8(heightinradarsystem.win.linehi-heightinradarsystem.win.linelo+1) / multiL);
	  final int32 mlpixels = Math.floor(real8(heightinradarsystem.win.pixhi-heightinradarsystem.win.pixlo+1) / multiP);

	  // ______ Line/pixel of first point in original master coordinates ______
	  final real8 veryfirstline = real8(heightinradarsystem.win.linelo) + (multiL - 1.) / 2.;
	  final real8 firstpixel = real8(heightinradarsystem.win.pixlo) + (multiP - 1.) / 2.;


	// ====== Compute number of buffers required ======
	  final int32 NUMMAT = 3; // number of large matrices to determine size of matrices
	  int32 bufferlines = generalinput.memory / (NUMMAT * (mlpixels * sizeof(real4)));
	  if (bufferlines > mllines) // whole image fits in BUFFER
		bufferlines = mllines;
	  ...
	  // ______ Window to be read into BUFFER from file in multilooked system ______
	  matrix<real4> PHI = new matrix(bufferlines,mlpixels);
	  matrix<real4> LAMBDA = new matrix(bufferlines,mlpixels);
	  matrix<real4> HEIGHT = new matrix(bufferlines,mlpixels); // also output
	  final uint dummy = 999999; // large to force error if not ok
	  window bufferwin = new window(1, bufferlines, 1, mlpixels); // initial
	  window offsetbuffer = new window(1,dummy,1,dummy); // dummy not used in readfile, no offset
	...
	// ====== Process BUFFERS ======
	  for (register int32 buffer =1; buffer<=FULLBUFFERS+EXTRABUFFER; buffer++)
		{
		// ______ Give progress ______
		PROGRESS << "GEOCODE: " << 100*(buffer-1)/(FULLBUFFERS+EXTRABUFFER) << "%";
		PROGRESS.print();
		...
		switch (heightinradarsystem.formatflag)
		  {
		  case FORMATR4:
			readfile (HEIGHT, heightinradarsystem.file, mllines, bufferwin, offsetbuffer);
			break;
...
//ORIGINAL LINE: register real8 pixel = firstpixel - multiP;
	...
	// ====== Write to result files ======
	  ofstream scratchlogfile = new ofstream("scratchloggeocode", ios.out | ios.trunc);
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  bk_assert(scratchlogfile,"geocode: scratchloggeocode",__FILE__,__LINE__);
	  scratchlogfile << "\n\n*******************************************************************" << "\n* " << processcontrol[pr_i_geocoding] << "\n*******************************************************************" << "\nData_output_file_hei (slant2h): " << heightinradarsystem.file << "\nData_output_file_phi: \t\t" << geocodeinput.fophi << "\nData_output_file_lambda: \t" << geocodeinput.folam << "\n";
	  scratchlogfile.close();


	  ofstream scratchresfile = new ofstream("scratchresgeocode", ios.out | ios.trunc);
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  bk_assert(scratchresfile,"geocode: scratchresgeocode",__FILE__,__LINE__);
	  scratchresfile << "\n\n*******************************************************************" << "\n*_Start_" << processcontrol[pr_i_geocoding] << "\n*******************************************************************" << "\nData_output_file_hei (slant2h): " << heightinradarsystem.file << "\nData_output_file_phi:                 \t" << geocodeinput.fophi << "\nData_output_file_lamda:               \t" << geocodeinput.folam << "\nData_output_format:                   \t" << "real4" << "\nFirst_line (w.r.t. original_master):  \t" << heightinradarsystem.win.linelo << "\nLast_line (w.r.t. original_master):   \t" << heightinradarsystem.win.linehi << "\nFirst_pixel (w.r.t. original_master): \t" << heightinradarsystem.win.pixlo << "\nLast_pixel (w.r.t. original_master):  \t" << heightinradarsystem.win.pixhi << "\nMultilookfactor_azimuth_direction:    \t" << heightinradarsystem.multilookL << "\nMultilookfactor_range_direction:      \t" << heightinradarsystem.multilookP << "\n*******************************************************************" << "\n* End_" << processcontrol[pr_i_geocoding] << "_NORMAL" << "\n*******************************************************************\n";
	  scratchresfile.close();
