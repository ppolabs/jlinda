public class GlobalMembersUnwrap
{
	...
	// ***************************************************************
	public static void unwraptreeframon(input_gen generalinput, input_unwrap unwrapinput, productinfo interferogram)
	  {
	  TRACE_FUNCTION("unwraptreeframon (BK 07-Jun-1999)")

	//  char prog[ONE27]="/home/hanssen/projects/cerroprieto/matlab/manuw/treef_ramon ";
	  String prog = "treef_ramon ";
	  String fileouttreeframon = "ramon.uw";
	  String dummyline = new String(new char[ONE27]);


	  final int32 dL = unwrapinput.deltaLseed;
	  final int32 dP = unwrapinput.deltaPseed;
	//  const int32 dL = 100;
	//  const int32 dP = dL;
	...
	// ______ Number of lines/pixels on file (complex interferogram) ______
	  final int32 fileliness = (interferogram.win.linehi - interferogram.win.linelo + 1) / interferogram.multilookL;
	  final int32 filepixels = (interferogram.win.pixhi - interferogram.win.pixlo + 1) / interferogram.multilookP;
	...
	//  matrix<complr4>     AMP_PHA(fileliness,filepixels);         // read from file ramon.uw
	  matrix<real4> AMP = new matrix(fileliness, filepixels); // read from file ramon.uw
	  matrix<real4> PHA = new matrix(fileliness, filepixels); // read from file ramon.uw
	  matrix<int16> REGIONS = new matrix(fileliness, filepixels); // initialize to 0
	...
	//  matrix<real4>       PHASE(fileliness,filepixels,NaN);       // initialize to NaN
	  matrix<real4> PHASE = new matrix(fileliness, filepixels);
	  PHASE.setdata(NaN);


	// ______ Make commandstring ______
	  String basecmdstring = new String(new char[3 *ONE27]);
	  ostrstream basecmdstr = new ostrstream(basecmdstring,3 *ONE27); // to convert int to char
	  basecmdstr.seekp(0);
	//           << filepixels << " " << fileliness << " 1 0  ";    // FIRSTLINE = 0 ! (see source)
	  basecmdstr << prog << interferogram.file << " " << filepixels << " " << fileliness << " 0 0  "; // no ends!!
	  final int32 posbasecmdstr = basecmdstr.tellp();
	...
	// ______ Windows for band interleaved file loading, cmp readhgt ______
	  ...
	  window winamp = new window(1, fileliness, 1, filepixels);
	  window winpha = new window(1, fileliness, filepixels+1, 2 *filepixels);
...
//ORIGINAL LINE: register int32 regionnumber = 0;
	  int32 regionnumber = 0;
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register int32 line, pixel;
	  int32 line;
	  int32 pixel;
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register int32 npoints = 0;
	  int32 npoints = 0;
	  uint seedL;
	  uint seedP;
	  matrix<uint> SEEDS;
	  if (Character.isWhitespace(unwrapinput.seedfile[0])) // no filename specified
		{
...
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register int32 cnt = 0;
		int32 cnt = 0;
		for (seedL =4; seedL<=fileliness-4; seedL =seedL+dL)
		  {
		  for (seedP =4; seedP<=filepixels-4; seedP =seedP+dP)
		...
		INFO << "Number of points in seedfile (should be an enter after last one): " << npoints;
		INFO.print();
		ifstream ifile = new ifstream(unwrapinput.seedfile, ios.in); // not binary
		SEEDS.resize(npoints, 2);
	...
	// ______ Use amp for check if unwrapped ok.
		  readfile (AMP, fileouttreeframon, AMP.lines(), winamp, offsetbuffer);
		  readfile (PHA, fileouttreeframon, PHA.lines(), winpha, offsetbuffer);
	...
	// ______ Remove regions that are not ok unwrapped (too small) ______
	  final int32 SEVENTY = 70; // criterium
	  final int32 maxregions = regionnumber;
	  int32 numberofregions = regionnumber;
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register int32 numberofpixels = 0;
	  int32 numberofpixels = 0;
	...
	// ====== Write result to output files ======
	  ...
	  RefObject<ofstream> TempRefObject = new RefObject<ofstream>(ofileregions);
	  openfstream(TempRefObject, unwrapinput.foregions, generalinput.overwrit);
	  ofileregions = TempRefObject.argvalue;
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  ofstream ofileuint;
	  RefObject<ofstream> TempRefObject2 = new RefObject<ofstream>(ofileuint);
	  openfstream(TempRefObject2, unwrapinput.fouint, generalinput.overwrit);
	  ofileuint = TempRefObject2.argvalue;
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	...
	// ====== Write info to resultfiles ======
	  ofstream scratchlogfile = new ofstream("scratchlogunwrap", ios.out | ios.trunc);
	  scratchlogfile << "\n\n*******************************************************************" << "\n**** UNWRAP                                                   *****" << "\n*******************************************************************" << "\nData_output_file: \t\t" << unwrapinput.fouint << "\nData_output_format: \t\t" << "real4" << "\nData_output_file_regions: \t" << unwrapinput.foregions << "\nData_output_format: \t\t" << "short int (2B)" << "\nProgram for unwrappng: \t\t" << prog << "\nOutput program for unwrapping: \t" << fileouttreeframon;
	  if (Character.isWhitespace(unwrapinput.seedfile[0])) // no seedfile specified
		{
		scratchlogfile << "\nDelta lines for seed: \t\t" << dL << "\nDelta pixels for seed: \t\t" << dP;
		}
	  else
		{
		scratchlogfile << "\nSeeds where read from file: \t" << unwrapinput.seedfile;
		}
	  scratchlogfile << "\nNumber of patches used: \t" << numberofregions << "\n* End_unwrap:" << "\n*******************************************************************\n";
	  scratchlogfile.close();

	  ofstream scratchresfile = new ofstream("scratchresunwrap", ios.out | ios.trunc);
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	...
	// ***************************************************************
	public static void snaphu_unwrap(input_gen generalinput, input_unwrap unwrapinput, productinfo interferogram, slcimage master, slcimage slave, RefObject<orbit> masterorbit, RefObject<orbit> slaveorbit, input_ell ellips)
	  {
	  TRACE_FUNCTION("snaphu_unwrap (Bert Kampes 01-Nov-2002)")
	  String prog = "snaphu "; // run this executable
	  String configfile = "snaphu.conf"; // create this file
	  INFO << "Configuration file for snaphu: " << configfile;
	  INFO.print();


	  // ______ Number of lines/pixels on file (complex interferogram) ______
	  final int32 filelines = (interferogram.win.linehi - interferogram.win.linelo + 1) / interferogram.multilookL;
	  final int32 filepixels = (interferogram.win.pixhi - interferogram.win.pixlo + 1) / interferogram.multilookP;
	  ...
	  ostrstream basecmdstr = new ostrstream(basecmdstring,3 *ONE27); // to convert int to char
	  basecmdstr.seekp(0);
	  basecmdstr << prog << " -f " << configfile << " " << interferogram.file << " " << filepixels << ends;
	  ...
	  real8 DR = new real8(interferogram.multilookP);
		//(pix2range(interferogram.win.pixlo+1,master.t_range1,master.rsr2x) - NEARRANGE);
	  real8 RANGERES = ((master.rsr2x/2.)/master.rbw) * (DR/real8(interferogram.multilookP));
	  ...
	  lp2xyz(line,pixel,ellips,master,masterorbit.argvalue,P); // returns P
	  real8 EARTHRADIUS = P.norm();
	  // ______ Compute xyz for master satellite ______
	  real8 time_azi; // returned
	  real8 time_range; // returned
	  xyz2t(time_azi,time_range,master, masterorbit.argvalue, P);
	  cn M = masterorbit.argvalue.getxyz(time_azi); // master satellite position
	  xyz2t(time_azi,time_range,slave, slaveorbit.argvalue, P);
	  cn S = slaveorbit.argvalue.getxyz(time_azi); // slave satellite position
	  ...
	  // ====== Write info to resultfiles ======
	  ofstream scratchlogfile = new ofstream("scratchlogunwrap", ios.out | ios.trunc);
	  scratchlogfile << "\n\n*********************************************************" << "\n**** UNWRAP                                         *****" << "\n*********************************************************" << "\nData_output_file: \t\t" << unwrapinput.fouint << "\nData_output_format: \t\t" << "hgt or real4" << "\nProgram for unwrappng: \t\t" << prog << "\n* End_unwrap:" << "\n*********************************************************\n";
	  scratchlogfile.close();
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  scratchresfile << "\n\n*******************************************************************" << "\n*_Start_" << processcontrol[pr_i_unwrap] << "\n*******************************************************************" << "\nData_output_file:                     \t" << unwrapinput.fouint << "\nData_output_format:                   \t";
		...
		if (unwrapinput.oformatflag == FORMATHGT)
		  ...
		  throw(unhandled_case_error);
	  scratchresfile << "\nFirst_line (w.r.t. original_master):  \t" << interferogram.win.linelo << "\nLast_line (w.r.t. original_master):   \t" << interferogram.win.linehi << "\nFirst_pixel (w.r.t. original_master): \t" << interferogram.win.pixlo << "\nLast_pixel (w.r.t. original_master):  \t" << interferogram.win.pixhi << "\nMultilookfactor_azimuth_direction:    \t" << interferogram.multilookL << "\nMultilookfactor_range_direction:      \t" << interferogram.multilookP << "\nNumber of lines (multilooked):        \t" << filelines << "\nNumber of pixels (multilooked):       \t" << filepixels << "\nProgram used for unwrapping:          \t" << prog << "\n*******************************************************************" << "\n* End_" << processcontrol[pr_i_unwrap] << "_NORMAL" << "\n*******************************************************************\n";
	  scratchresfile.close();
