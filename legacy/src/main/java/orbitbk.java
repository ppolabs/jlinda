public class GlobalMembersOrbitbk
{
	//
...
// ***************************************************************
	...
	public static matrix<real8> splineinterpol(matrix<real8> time, matrix<real8> data)
	  {
	  ...
	  final int32 N = time.lines(); // number of points
	  matrix<real8> rhs = new matrix(N-1, 1);
	  matrix<real8> pp = new matrix(N, 1); // init to 0.

	  // #define __NATURALSPLINE__ // in Makefile or here if so desired.
	//#define __NATURALSPLINE__
	  // ====== Set boundary condition first datapoint ======
	//C++ TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	//  #if __NATURALSPLINE__
		pp(0,0) = 0.0; // natural spline boundary condition register int32 i;
	//  #else
		// ______ yp1 is first der. at point 0 ______
		// estimate it by dy/dx: nearly linear for sat. orbit...
		final real8 yp1 = (data(1,0)-data(0,0)) / (time(1,0)-time(0,0));
		pp(0,0) = -0.5;
		rhs(0,0) = (3.0 / (time(1,0)-time(0,0))) * ((data(1,0)-data(0,0)) / (time(1,0)-time(0,0)) - yp1);
...
//ORIGINAL LINE: register int32 i;
	...
	//  #endif
	  ...
	  for (i =1; i<=N-2; ++i)
		...
		final int32 ip1 = i + 1;
		final int32 im1 = i - 1;
		final real8 sig = (time(i,0)-time(im1,0)) / (time(ip1,0)-time(im1,0));
		final real8 p = sig *pp(im1,0) + 2.0;
		//
		pp(i,0) = (sig-1.0) / p;
	...
	//  #else
		...
		final real8 ypN = (data(N-1,0)-data(N-2,0)) / (time(N-1,0)-time(N-2,0));
		final real8 qn = 0.5;
		final real8 un = (3.0 / (time(N-1,0) - time(N-2,0))) * (ypN - (data(N-1,0) - data(N-2,0)) / (time(N-1,0) - time(N-2,0)));
...
// ***************************************************************
	public static matrix<real8> polyfit(matrix<real8> time, matrix<real8> y, int32 DEGREE)
	  {
	  ...
	  final int32 Npoints = time.lines(); // number of points
	  ...
	  solvechol(Qx_hat, TempRefObject2); // Estimate of unknowns in rhs
	  rhs = TempRefObject2.argvalue;
	  RefObject<matrix<real4>> TempRefObject3 = new RefObject<matrix<real4>>(Qx_hat);
	  invertchol(TempRefObject3); // Covariance matrix
	  ...
	  if (maxMath.abs(e_hat) > 0.02) // 0.05 is already 1 wavelength! (?)
		{
		WARNING << "Max. approximation error at datapoints (x,y,or z?): " << maxMath.abs(e_hat) << "m";
		WARNING.print();
		}
	  else
		{
		INFO << "Max. approximation error at datapoints (x,y,or z?): " << maxMath.abs(e_hat) << "m";
		INFO.print();
		}
	  ...
	  for (int32 i =0; i<Npoints; i++)
		{
		DEBUG << setw(16) << setprecision(15) << time(i,0) << "   " << y(i,0) << "   " << y_hat(i,0) << "   " << e_hat(i,0);
		DEBUG.print();
		}
	  ...
	  for (int32 i =0; i<Npoints-1; i++)
		...
		DEBUG.print();
		if(Math.abs(dt-(time(1,0)-time(0,0))) > 0.001) // 1ms i will allow...
...
//ORIGINAL LINE: void orbit::initialize(String file)
	...
	// ***************************************************************
//C++ TO JAVA CONVERTER WARNING: The declaration of the following method implementation was not found:
//ORIGINAL LINE: void orbit::computecoefficients()
	...
	// ***************************************************************
//C++ TO JAVA CONVERTER WARNING: The declaration of the following method implementation was not found:
//ORIGINAL LINE: void orbit::getklokhi(real8 t)
	...
	// ***************************************************************
//C++ TO JAVA CONVERTER WARNING: The declaration of the following method implementation was not found:
//ORIGINAL LINE: cn orbit::getxyz(real8 t)
 // const // not, klo,khi
	...
	// ***************************************************************
//C++ TO JAVA CONVERTER WARNING: The declaration of the following method implementation was not found:
//ORIGINAL LINE: cn orbit::getxyzdot(real8 t)
	...
	// ***************************************************************
//C++ TO JAVA CONVERTER WARNING: The declaration of the following method implementation was not found:
//ORIGINAL LINE: cn orbit::getxyzddot(real8 t)
//C++ TO JAVA CONVERTER NOTE: This was formerly a static local variable declaration (not allowed in Java):
private matrix<real8> solxyz = new matrix(3,1);
//C++ TO JAVA CONVERTER NOTE: This was formerly a static local variable declaration (not allowed in Java):
private matrix<real8> equationset = new matrix(3,1);
//C++ TO JAVA CONVERTER NOTE: This was formerly a static local variable declaration (not allowed in Java):
private matrix<real8> partialsxyz = new matrix(3,3);
	...
	// ***************************************************************
	public static int32 lp2xyz(real8 line, real8 pixel, input_ell ell, slcimage image, RefObject<orbit> orb, RefObject<cn> returnpos, int32 MAXITER, real8 CRITERPOS)
	  {
	  TRACE_FUNCTION("lp2xyz (BK 04-Jan-1999)")

	  // ______ Convert lp to time ______
	  final real8 aztime = image.line2ta(line);
	  final real8 ratime = image.pix2tr(pixel);

	  final cn possat = orb.argvalue.getxyz(aztime);
	  final cn velsat = orb.argvalue.getxyzdot(aztime);

	  // ______ Set up system of equations and iterate ______
	  returnpos.argvalue.x = image.approxcentreoriginal.x; // iteration 0
	  returnpos.argvalue.y = image.approxcentreoriginal.y; // iteration 0
	  returnpos.argvalue.z = image.approxcentreoriginal.z; // iteration 0
...
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register int32 iter;
	  int32 iter;
	  for (iter =0; iter<=MAXITER; ++iter)
		{

		// ______ Update equations + solve system ______
		final cn dsat_P = returnpos.argvalue.min(possat); // vector satellite, P on ellipsoid
		equationset(0,0) = -eq1_doppler(velsat, dsat_P);
		equationset(1,0) = -eq2_range(dsat_P, ratime);
		equationset(2,0) = -eq3_ellipsoid(returnpos.argvalue,ell.a,ell.b);
		...
		partialsxyz(2,0) = (2 *returnpos.argvalue.x)/sqr(ell.a);
		partialsxyz(2,1) = (2 *returnpos.argvalue.y)/sqr(ell.a);
		partialsxyz(2,2) = (2 *returnpos.argvalue.z)/sqr(ell.b);

		// ______ Solve system ______
		solve33(solxyz, equationset,partialsxyz);

		// ______Update solution______
		returnpos.argvalue.x += solxyz(0,0); // update approx. value
		returnpos.argvalue.y += solxyz(1,0); // update approx. value
		returnpos.argvalue.z += solxyz(2,0); // update approx. value

		// ______Check convergence______
		if (Math.abs(solxyz(0,0)) < CRITERPOS && Math.abs(solxyz(1,0)) < CRITERPOS && Math.abs(solxyz(2,0)) < CRITERPOS) // dz
		  break; // converged
		}
	  ...
	  if (iter >= MAXITER)
		{
		WARNING << "line, pix -> x,y,z: maximum iterations (" << MAXITER << ") reached. " << "Criterium (m): "<< CRITERPOS << "dx,dy,dz=" << solxyz(0,0) << ", " << solxyz(1,0) << ", " << solxyz(2,0);
		WARNING.print();
		}
	...
	// #%// BK 22-Sep-2000                                            *
	// ***************************************************************
	public static int32 xyz2orb(RefObject<cn> possat, slcimage image, RefObject<orbit> orb, cn pointonellips, int32 MAXITER, real8 CRITERTIM) // seconds
	  {
...
//ORIGINAL LINE: register int32 iter;
	  ...
	  for(iter =0 ;iter<=MAXITER; ++iter) // break at convergence
		...
		possat.argvalue = orb.argvalue.getxyz(tazi);
		final cn velsat = orb.argvalue.getxyzdot(tazi);
		final cn accsat = orb.argvalue.getxyzddot(tazi);
		final cn delta = pointonellips.min(possat.argvalue);

		// ______ Update solution ______
		sol = -eq1_doppler(velsat, delta) / eq1_doppler_dt(delta, velsat, accsat);
		tazi += sol;

		// ______ Check convergence ______
		if (Math.abs(sol) < CRITERTIM) // dta
		  break;
		}
	  ...
	  if (iter >= MAXITER)
		{
		WARNING << "x,y,z -> line, pix: maximum iterations (" << MAXITER << ") reached. " << "Criterium (s):" << CRITERTIM << "dta (s)=" << sol;
		WARNING.print();
		}
	...
	// *    Bert Kampes, 04-Jan-1999                                  *
	// ***************************************************************
	public static int32 xyz2t(RefObject<real8> tazi, RefObject<real8> tran, slcimage image, RefObject<orbit> orb, cn pos, int32 MAXITER, real8 CRITERTIM) // seconds
	  {
	  TRACE_FUNCTION("xyz2t (BK 04-Jan-1999)")

	  // ______ Compute initial value ______
	  tazi.argvalue = image.line2ta(0.5*(image.currentwindow.linehi-image.currentwindow.linelo));
	  // _____ Start _______
	  real8 sol = 0.0;
//C++ TO JAVA CONVERTER NOTE: 'register' variable declarations are not supported in Java:
//ORIGINAL LINE: register int32 iter;
	  ...
	  for(iter =0 ;iter<=MAXITER; ++iter) // break at convergence
		{
		// ______ Update equations ______
		final cn possat = orb.argvalue.getxyz(tazi.argvalue);
		final cn velsat = orb.argvalue.getxyzdot(tazi.argvalue);
		final cn accsat = orb.argvalue.getxyzddot(tazi.argvalue);
		final cn delta = pos.min(possat);
	...
	// *    Bert Kampes, 04-Jan-1999                                  *
	// ***************************************************************
	public static int32 xyz2lp(RefObject<real8> returnline, RefObject<real8> returnpixel, slcimage image, RefObject<orbit> orb, cn pos, int32 MAXITER, real8 CRITERTIM)
	  {
	  ...
	  RefObject<real8> TempRefObject = new RefObject<real8>(tazi);
	  RefObject<real8> TempRefObject2 = new RefObject<real8>(tran);
	  int32 iter = xyz2t(TempRefObject, TempRefObject2, image, orb, pos, MAXITER, CRITERTIM);
	  tazi = TempRefObject.argvalue;
	  tran = TempRefObject2.argvalue;

	  // ______ Convert time to pixel ______
	  // ______ (Converged) Result is in returnline/pixel ______
	  returnline.argvalue = image.ta2line(tazi);
	  returnpixel.argvalue = image.tr2pix (tran);
	  return iter;
	  } // END xyz2lp
	...
	// ***************************************************************
	public static int32 ell2lp(RefObject<real8> returnline, RefObject<real8> returnpixel, input_ell ellips, slcimage image, RefObject<orbit> orb, real8 phi, real8 lambda, real8 height, int32 MAXITER, real8 CRITERTIM)
	  {
	  TRACE_FUNCTION("ell2lp (BK 27-Jan-1999)")
	  // ______ Transform ell2xyz ______
	  cn returnpos = ellips.ell2xyz(phi, lambda, height);
	  DEBUG << "tmp result phi,lambda,h --> x,y,z: " << phi << ", " << lambda << ", " << height << " --> " << returnpos.x << " " << returnpos.y << " " << returnpos.z;
	  DEBUG.print();
	  // ______ Transform xyz2lp ______
	  int32 n_iter = xyz2lp(returnline, returnpixel, image, orb, returnpos, MAXITER, CRITERTIM);
	...
	// *    Bert Kampes, 27-Jan-1999                                  *
	// ***************************************************************
	public static int32 lp2ell(real8 line, real8 pixel, input_ell ellips, slcimage image, RefObject<orbit> orb, RefObject<real8> returnphi, RefObject<real8> returnlambda, RefObject<real8> returnheight, int32 MAXITER, real8 CRITERPOS)
	  {
	  TRACE_FUNCTION("lp2ell (BK: 27-Jan-1999)");
	  // ______ Transform lp2xyz ______
	  cn returnpos;
	  RefObject<cn> TempRefObject = new RefObject<cn>(returnpos);
	  int32 iter = lp2xyz(line, pixel, ellips, image, orb, TempRefObject, MAXITER, CRITERPOS);
	  returnpos = TempRefObject.argvalue;
	  // ______ Transform xyz2ell, compute phi,lambda,h ______
	  ellips.xyz2ell(returnpos, returnphi.argvalue, returnlambda.argvalue, returnheight.argvalue);
	  return iter;
	  } // END lp2ell
	...
	// ***************************************************************
//C++ TO JAVA CONVERTER WARNING: The declaration of the following method implementation was not found:
//ORIGINAL LINE: void orbit::dumporbit(const input_pr_orbits &inputorb, const int16 ID)
	...
	// ***************************************************************
//C++ TO JAVA CONVERTER WARNING: The declaration of the following method implementation was not found:
//ORIGINAL LINE: void orbit::showdata()
	...
	static int32 Main()
	  ...
	  cerr << "pos(" << t << "): " << pos.x << ", " << pos.y << ", " << pos.z << "\n";
	  cerr << "vel(" << t << "): " << vel.x << ", " << vel.y << ", " << vel.z << "\n";
	  cerr << "acc(" << t << "): " << acc.x << ", " << acc.y << ", " << acc.z << "\n";
...
//ORIGINAL LINE: register int32 i;
	...
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  if (!foundsection)
		{
		WARNING << "string: \"NUMBER_OF_DATAPOINTS:\" not found (i.e., orbit) in file: " << file << ".";
		WARNING.print();
		}
	  INFO << numberofpoints << " datapoints (t,x,y,z) read from: \"" << file << "\"";
	  INFO.print();
	  ...
	  if (foundsection)
		...
		for (i =0; i<numberofpoints-1; ++i)
		  {
		  final real8 h = time(i+1,0) - time(i,0); // delta_t, might not be const.
		  if (h < EPS) // ~= 0.)
			{
	...
	public void computecoefficients()
	  ...
	  switch (interp_method) // either spline method or degree of polynomial
		...
		case ORB_SPLINE:
		  ...
		  break;
		default:
		  INFO << "Computing coefficients for orbit polyfit degree: " << interp_method;
		  INFO.print();
		  if (interp_method < 0)
			{
	...
	//#if __DEBUG
	  ...
	  if (t < time(0,0) || t > time(numberofpoints-1,0))
		{
		WARNING << "interpolation at: " << t << " is outside interval time axis: (" << time(0,0) << ", " << time(numberofpoints-1,0) << ").";
		WARNING.print();
		}
	//#endif
	  ...
	  if (interp_method==ORB_SPLINE)
		...
		final real8 h = time(khi,0) - time(klo,0); // delta_t, might not be const.
		final real8 a = (time(khi,0) - t) / h;
		final real8 b = 1. - a;
		// ______ Evaluate function in x,y,z ______
		satpos.x = a *data_x(klo,0)+b *data_x(khi,0) + (((sqr(a)*a-a)*coef_x(klo,0)+(sqr(b)*b-b)*coef_x(khi,0))*sqr(h))/6.;
		satpos.y = a *data_y(klo,0)+b *data_y(khi,0) + (((sqr(a)*a-a)*coef_y(klo,0)+(sqr(b)*b-b)*coef_y(khi,0))*sqr(h))/6.;
		satpos.z = a *data_z(klo,0)+b *data_z(khi,0) + (((sqr(a)*a-a)*coef_z(klo,0)+(sqr(b)*b-b)*coef_z(khi,0))*sqr(h))/6.;
		}
	  ...
	  // ______ (it seems else vdot=vdot*.5(min+max) for some reason)
		...
		{
		final real8 t_tmp = (t-time(numberofpoints/2,0))/real8(10.0);
	...
	//#endif
	  ...
	  if (interp_method==ORB_SPLINE)
		...
		satvel.x = ((data_x(khi,0)-data_x(klo,0)) / h) + (h * (((1-(3 *sqr(a)))*coef_x(klo,0)) + (((3 *sqr(b))-1)*coef_x(khi,0)))) / 6.;
		satvel.y = ((data_y(khi,0)-data_y(klo,0)) / h) + (h * (((1-(3 *sqr(a)))*coef_y(klo,0)) + (((3 *sqr(b))-1)*coef_y(khi,0)))) / 6.;
		satvel.z = ((data_z(khi,0)-data_z(klo,0)) / h) + (h * (((1-(3 *sqr(a)))*coef_z(klo,0)) + (((3 *sqr(b))-1)*coef_z(khi,0)))) / 6.;
		}
	  // ______ Orbit interpolator is simple polynomial ______
	  else
		{
		final int32 DEGREE = coef_x.lines()-1;
	...
	public void dumporbit(input_pr_orbits inputorb, int16 ID)
	  {
	  ...
	  real8 dt =0.;
	  String ofile = new String(new char[EIGHTY]);
	  if (ID ==MASTERID)
		{
		if (inputorb.dumpmasterorbit<0)
			return;
		dt = inputorb.dumpmasterorbit;
		ofile = "masterorbit.dat";
	  ...
	  INFO << "dumporbits: MAXITER: " << MAXITER << "; " << "CRITERPOS: " << CRITERPOS << " m; " << "CRITERTIM: " << CRITERTIM << " s";
	  INFO.print();

	  //  ______ Evaluate polynomial orbit for t1:dt:tN ______
	  int32 outputlines = 1 + int32((time(numberofpoints-1,0)-time(0,0)) / dt);
	  real8 tazi = time(0,0);
	  ofstream fo = new ofstream(ofile,ios.out | ios.trunc);
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __LINE__ macro:
	//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent in Java to the C++ __FILE__ macro:
	  ...
	  for (register int32 i =0; i<outputlines; ++i)
		{
		final cn position = getxyz(tazi);
		final cn velocity = getxyzdot(tazi);
		final cn accerela = getxyzddot(tazi);
		fo << tazi << " " << position.x << " " << position.y << " " << position.z << " " << velocity.x << " " << velocity.y << " " << velocity.z << " " << accerela.x << " " << accerela.y << " " << accerela.z << "\n";
		tazi += dt;
		}
	  fo.close();
