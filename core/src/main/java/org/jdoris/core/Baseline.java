...
*/
    ...
    boolean initialized;        //
    double master_wavelength;   // tmp for now used for h_amb
    double nearrange;           // range=nearrange+drange_dp*pixel
    double drange_dp;           // range=nearrange+drange_dp*pixel
    double orbit_convergence;   //    tmp for now constant
    double orbit_heading;       //    tmp for now NOT USED
    double L_min;               // for normalization
    double L_max;               // for normalization
    double P_min;               // for normalization
    double P_max;               // for normalization
    double H_min;               // height at which parameters are modeled
    double H_max;               // to model phase=f(line,pix,hei) and hei=g(line,pix,phase)

    long N_coeffs;              // ==10th degree of 3D-poly to model B(l,p,h)
                                // --- B(l,p,h) = a000 +
                                //                a100*l   + a010*p   + a001*h   +
                                //                a110*l*p + a101*l*h + a011*p*h +
                                //                a200*l^2 + a020*p^2 + a002*h^2
    ...
    // --- Coefficients ---
    DoubleMatrix BPERP_cf;          // perpendicular baseline
    DoubleMatrix BPAR_cf;           // parallel baseline
    DoubleMatrix THETA_cf;          // viewing angle
    DoubleMatrix THETA_INC_cf;      // incidence angle to satellite
   ...
   ****************************************************************/
     ...
     * **************************************************************/
    public void estimateBBparBperpTheta() {
        /*
     ...
     * **************************************************************
     */
    public void estimateIncidenceAngle() {
  /*real8 IncidenceAngle(const cn Master, const cn Point) const
    ...
    private static void modelParameters(){
            //
